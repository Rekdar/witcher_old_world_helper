...
The component renders a two-column layout: left column has the token image + draw buttons + Skellige toggle + "Usuń zaznaczenie" button (visible only when a token is drawn); right column shows `map.jpg` permanently with overlaid markers:
- **Red circle marker** (`rgba(220,53,69,0.85)`, `clamp(42px,9%,84px)`) — appears at the drawn token's map position while a token is selected; hidden after "Usuń zaznaczenie" or after saving a task. Uses `TOKEN_MAP_POSITIONS` from `terrains.tsx`. Not persisted to localStorage.
- **Quest markers** (`src/img/inventory/quest.png`, `clamp(7px,1.5%,14px)`) — one per saved task that has a map position, rendered below the red marker (top of quest icon aligns with center of red marker). Multiple tasks at the same location are offset +3% horizontally per additional marker. Markers disappear when their task is marked Done. Quest markers are persisted via `locationTokens_tasks`.
- The map supports click-to-zoom via the existing `enlargedImage` modal; `enlargedImageIsMap` flag disables `roundedCircle` on the modal image for the rectangular map.
- Adding new terrain expansions: add coordinates to `TOKEN_MAP_POSITIONS` in `terrains.tsx` if the expansion tokens have base-game map locations.
